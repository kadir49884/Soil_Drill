...
public class RuntimeCircleClipper : MonoBehaviour, IClip
{
    ...
    private float drillAxisY;
    private float terrainAxisY;
    [SerializeField] private bool isFinish = false;


    [SerializeField] private GameObject drillPos;
    ...
    void Start()
    {
        drillPos = ObjectManager.Instance.SpearParent.GetChild(0).GetChild(0).gameObject;
    }
    ...
    void UpdateTouch()
    {
        ...
        Vector2 XOYPlaneLocation = (drillPos.transform.position);

        //if (terrain.gameObject.GetComponent<DestructibleTerrain>().IsHorizontal)
        //{
            drillAxisY = drillPos.transform.position.z;
            terrainAxisY = terrain.transform.position.z;
        //}
        //else
        //{
        //    drillAxisY = drillPos.transform.position.y;
        //    terrainAxisY = terrain.transform.position.y;
        //}
        Vector3 finishPos = drillPos.transform.position;
        Vector2 objectPos = new Vector2(drillPos.transform.position.y, drillAxisY);
        Vector2 planePos = new Vector2(terrain.transform.position.y, terrainAxisY);


        //planePos.y -= 0.2f;

        currentTouchPoint = objectPos - planePos;
    ...
    public void TerrainChangeParent()
    {
        GameObject temp = ObjectManager.Instance.SpearParent;
        if (temp.transform.childCount > 2)
        {
            drillPos = temp.GetChild(temp.transform.childCount - 2).gameObject;
        }


    }
    public void StackTerrainChangeParent()
    {
        GameObject temp = ObjectManager.Instance.SpearParent;
        if (temp.transform.childCount > 2)
        {
            drillPos = temp.GetChild(temp.transform.childCount - 1).gameObject;
