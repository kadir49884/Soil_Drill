...
public class DestructibleTerrain : MonoBehaviour
{
    ...
    [SerializeField] private bool finishWood = false;
    ...
    public void Initialize()
    {
        ...
        for (int x = 0; x < resolutionX; x++)
        {
            for (int y = 0; y < resolutionY; y++)
            {
                ...
                if (!finishWood)
                {
                    block.transform.rotation *= Quaternion.Euler(-180, 90, 90);
                    block.transform.localScale = block.transform.localScale.SetX(-1);

                }
                else
                {
                    //block.transform.localScale = block.transform.localScale.SetZ(-1);
                }
            }
        }
    }
